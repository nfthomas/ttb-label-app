...
def generate_image(test_case: Dict[str, Any], html_content: str, output_dir: Path) -> Path:
...
def generate_all(config_path: Path, template_dir: Path, output_dir: Path) -> int:
    ...
    for tc in test_cases:
        try:
            ...
            print(f"Error in test case {tc.get('id', 'unknown')}: missing key {e}", file=sys.stderr)
            return 1
        except Exception as e:
            print(f"Error generating test case {tc.get('id', 'unknown')}: {e}", file=sys.stderr)
            return 1
